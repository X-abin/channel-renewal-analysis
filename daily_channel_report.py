...
class ApiClient(object):
    def __init__(self, base_url, api_user, authorization, cookie):
    ...
    def request_json(self, path, params, label):
        ...
        if self.authorization:
            command.extend(['-H', 'Authorization: %s' % self.authorization])
        elif self.cookie:
            command.extend(['-H', 'Cookie: %s' % self.cookie])
        if self.api_user:
            command.extend(['-H', 'New-Api-User: %s' % self.api_user])
        command.extend([
...
def telegram_text(payload):
    ...
    lines = [
        ...
        u'渠道数：%s｜请求量：%s｜成功率：%s' % (
            summary.get('channel_count', 0),
            summary.get('request_count', 0),
            ('%.1f%%' % (float(summary.get('success_rate') or 0) * 100)) if summary.get('success_rate') is not None else u'暂无',
        ),
        u'建议更换：%s｜暂不续费：%s｜谨慎续费：%s｜建议续费：%s' % (
            summary.get('replace_count', 0), summary.get('pause_count', 0),
            sum(1 for item in payload.get('channels', []) if item.get('recommendation') == u'谨慎续费'),
            sum(1 for item in payload.get('channels', []) if item.get('recommendation') == u'建议续费'),
        ),
...
def notion_create_page(payload, token, database_id):
    if not token or not database_id:
        raise RuntimeError('NOTION_TOKEN and NOTION_DATABASE_ID are required in daily.env')
    summary = payload.get('summary') or {}
    report_date = payload.get('report_date', '')
    success_rate = summary.get('success_rate')
    success_text = ('%.1f%%' % (float(success_rate) * 100)) if success_rate is not None else u'暂无'
    children = [
        {'object': 'block', 'type': 'heading_2', 'heading_2': {'rich_text': notion_rich_text(u'日报总览')}},
        {'object': 'block', 'type': 'paragraph', 'paragraph': {'rich_text': notion_rich_text(
            u'渠道数：%s｜请求量：%s｜成功率：%s｜建议更换：%s｜暂不续费：%s' % (
                summary.get('channel_count', 0), summary.get('request_count', 0), success_text,
                summary.get('replace_count', 0), summary.get('pause_count', 0))) }},
        {'object': 'block', 'type': 'paragraph', 'paragraph': {'rich_text': notion_rich_text(
            u'跳过异常渠道：%s' % len(payload.get('failed_channels', []))) }},
        {'object': 'block', 'type': 'heading_2', 'heading_2': {'rich_text': notion_rich_text(u'渠道明细')}},
    ]
    priority = {u'建议更换渠道': 0, u'暂不续费': 1, u'谨慎续费': 2, u'建议续费': 3}
    channels = sorted(payload.get('channels', []), key=lambda item: (priority.get(item.get('recommendation'), 9), -float(item.get('score') or 0), item.get('name') or u''))
    for item in channels:
    ...
    if process.returncode:
        raise RuntimeError('Notion request failed: %s' % output.decode('utf-8', 'replace')[:500])
    response = json.loads(output.decode('utf-8'))
...
def main():
    ...
    authorization = config.get('AUTHORIZATION') or config.get('AUTHORIZATION_HEADER', '')
    cookie = config.get('COOKIE')
    telegram_bot_token = config.get('TELEGRAM_BOT_TOKEN', '')
    telegram_chat_id = config.get('TELEGRAM_CHAT_ID', '')
    notion_token = config.get('NOTION_TOKEN') or config.get('NOTION_API_TOKEN', '')
    ...
    if not authorization and not cookie:
        raise RuntimeError('AUTHORIZATION (Bearer Token) is required in daily.env')
    ...
    client = ApiClient(base_url, api_user, authorization, cookie)
    ...
    def channel_worker():
        while True:
            ...
            try:
                ...
                with state_lock:
                    failed_channels.append({'id': channel.get('id'), 'name': channel.get('name') or (u'渠道 %s' % channel.get('id')), 'error': str(error)[:240]})
                    completed[0] += 1
                    print('channel skipped: %s (%s)' % (channel.get('id'), error))
                    sys.stdout.flush()
            finally:
